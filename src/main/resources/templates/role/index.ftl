	...
	<script src="${APP_PATH }/jquery/pagination/jquery.pagination.js"></script>
        <script type="text/javascript">
            ...
            $("tbody .btn-success").click(function(){
                window.location.href = "assignPermission";
            });
			...
			$("#checkAllBox").click(function(){
			 ...
			 function deleteBatch(){
            	...
            	layer.confirm("确定要删除选中的账号吗？",  {icon: 3, title:'提示'}, function(cindex){
    			    ...
    			    $.ajax({
     			    	type : "get",
     			    	url : "${APP_PATH}/role/doBatchDelete?id="+ids,
			...
			function toUpdate(id,pageno){
	            window.location.href="${APP_PATH}/role/toUpdate?id="+id+"&pageno="+pageno;
	        }
			...
			function doDelete(id,name){
            	layer.confirm("确定要删除"+name+"吗？",  {icon: 3, title:'提示'}, function(cindex){
    			    ...
    			    $.ajax({
     			    	type : "get",
     			    	url : "${APP_PATH}/role/doDelete?id="+id,
     			    	success : function(result){
     			    		if(result.success){
     			    			window.location.href="${APP_PATH}/role/index";
     			    		}else{
           ...
           function toAssignPermission(id){
        	   window.location.href="${APP_PATH }/role/toAssignPermission?id="+id;
           }
           var obj = {pagesize:5};
            	...
            	$.ajax({
	            	...
	            	success : function(result){
	            		...
	            		$.each(datas,function(i,n){
	            			...
	            			content+='  <td>';
	            			content+='	  <button type="button" onclick="toAssignPermission('+n.id+')" class="btn btn-success btn-xs"><i class=" glyphicon glyphicon-check"></i></button>';
