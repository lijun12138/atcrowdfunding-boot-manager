...
<button type="button" class="btn btn-primary" style="float:right;" onclick="window.location.href='${APP_PATH}/advert/toAdd'"><i class="glyphicon glyphicon-plus"></i> 新增</button>
<br>
 <hr style="clear:both;">
	...
	<script src="${APP_PATH }/jquery/pagination/jquery.pagination.js"></script>
        <script type="text/javascript">
			...
			$("#checkAllBox").click(function(){
			 ...
			 function deleteBatch(){
            	...
            	layer.confirm("确定要删除选中的账号吗？",  {icon: 3, title:'提示'}, function(cindex){
    			    ...
    			    $.ajax({
     			    	type : "get",
     			    	url : "${APP_PATH}/advert/doBatchDelete?id="+ids,
			...
			function toUpdate(id,pageno){
	            window.location.href="${APP_PATH}/advert/toUpdate?id="+id+"&pageno="+pageno;
	        }
			...
			function doDelete(id,name){
            	layer.confirm("确定要删除"+name+"吗？",  {icon: 3, title:'提示'}, function(cindex){
    			    ...
    			    $.ajax({
     			    	type : "get",
     			    	url : "${APP_PATH}/advert/doDelete?id="+id,
     			    	success : function(result){
     			    		if(result.success){
     			    			window.location.href="${APP_PATH}/advert/index";
     			    		}else{
