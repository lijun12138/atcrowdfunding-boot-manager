...
<button type="button" class="btn btn-primary" style="float:right;" onclick="window.location.href='${APP_PATH}/cert/toAdd'"><i class="glyphicon glyphicon-plus"></i> 新增</button>
<br>
 <hr style="clear:both;">
	...
	<script src="${APP_PATH }/jquery/pagination/jquery.pagination.js"></script>
        <script type="text/javascript">
            ...
            function pageChange(pageno){
                //window.location.href="${APP_PATH }/user/queryPage?pageno="+pageno;
                queryPage(pageno);
      		}
            var obj = {pagesize:10};
            ...
            $("#checkAllBox").click(function(){
			 ...
			 function deleteBatch(){
            	...
            	layer.confirm("确定要删除选中的账号吗？",  {icon: 3, title:'提示'}, function(cindex){
    			    ...
    			    $.ajax({
     			    	type : "get",
     			    	url : "${APP_PATH}/cert/doBatchDelete?id="+ids,
     			    	success : function(result){
     			    		if(result.success){
     			    			window.location.href="${APP_PATH}/cert/index";
     			    		}else{
			...
			function toUpdate(id,pageno){
	            window.location.href="${APP_PATH}/cert/toUpdate?id="+id+"&pageno="+pageno;
	        }
			...
			function doDelete(id,name){
            	layer.confirm("确定要删除"+name+"吗？",  {icon: 3, title:'提示'}, function(cindex){
    			    ...
    			    $.ajax({
     			    	type : "get",
     			    	url : "${APP_PATH}/cert/delete?id="+id,
     			    	success : function(result){
     			    		if(result.success){
     			    			window.location.href="${APP_PATH}/cert/index";
     			    		}else{
